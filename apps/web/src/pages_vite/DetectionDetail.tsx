...
export default function DetectionDetailPage() {
  ...
  const fromList = detections.find(d => d._id === pageParams.id) ?? null
  ...
  useEffect(() => {
    ...
    setLoading(true)
    apiGet<{ detection: Detection }>(`/detections/${pageParams.id}`)
      .then(data => {
  ...
  // Keep detection in sync when the list updates (e.g. socket update)
  useEffect(() => {
  ...
  if (loadError || !detection) {
    return (
      <div className="p-6">
        <button onClick={() => navigate('detections')} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-navy-900 mb-4">
        ...
        <div className="flex flex-wrap gap-3">
          {!existingCTI ? (
            ...
            <div className="flex items-center gap-2">
              <Button variant="secondary" onClick={() => navigate('cti-draft-editor', { id: existingCTI._id })}>
                <Globe size={15} />
