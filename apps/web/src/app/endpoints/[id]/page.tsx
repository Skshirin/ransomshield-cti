...
export default function EndpointDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (endpoint) return
    setLoading(true)
    apiGet<{ endpoint: Endpoint }>(`/endpoints/${endpointId}`)
      .then(data => {
  ...
  if (error || !endpoint) {
    return (
      <div className="p-6">
        <button onClick={goBack} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-navy-900 transition-colors mb-4 cursor-pointer">
