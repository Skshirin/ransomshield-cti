...
export default function DetectionsPage() {
  ...
  const handleRowClick = (detId: string) => {
    navigate('detection-detail', { id: detId })
    router.push(`/detections/${detId}`)
  }
    ...
    <div className="p-6 space-y-4">
      ...
      {detectionsError && (
        <div className="flex items-center gap-3 p-4 bg-red-50 border border-red-100 rounded-xl">
          ...
          <button
            onClick={refetchDetections}
            className="text-sm text-red-600 font-medium hover:text-red-800 underline cursor-pointer"
          >
            Retry
          </button>
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {filtered.map(det => (
                  <tr
                    key={det._id}
                    onClick={() => handleRowClick(det._id)}
